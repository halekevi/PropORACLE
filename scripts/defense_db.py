...
# These must NOT be stored in extra_json to avoid duplicate columns on read-back
_KNOWN = {
    "sport", "team", "updated_at",
...
def load_defense_from_db(
    ...
    try:
        ...
        # Drop keys from extra_json that already exist as fixed schema columns
        # to prevent duplicate columns that break .str accessors downstream
        existing_cols = set(df.columns) - {"extra_json", "sport"}
        extra_df = extra_df.drop(columns=[c for c in extra_df.columns if c in existing_cols], errors="ignore")
        df = pd.concat([df.drop(columns=["extra_json", "sport"], errors="ignore"), extra_df], axis=1)
